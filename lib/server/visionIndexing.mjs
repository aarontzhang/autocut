...
const DEFAULT_INDEX_SEGMENT_CONCURRENCY = clamp(Math.floor(HOST_PARALLELISM * 0.75), 2, 10);
const INDEX_SEGMENT_CONCURRENCY = normalizeConcurrency(
  process.env.ANALYSIS_INDEX_SEGMENT_CONCURRENCY,
  DEFAULT_INDEX_SEGMENT_CONCURRENCY,
);
const INDEX_DESCRIPTION_CONCURRENCY = normalizeConcurrency(
  process.env.ANALYSIS_INDEX_DESCRIPTION_CONCURRENCY,
  clamp(Math.ceil(DEFAULT_INDEX_SEGMENT_CONCURRENCY * 0.75), 2, 8),
);
const VERIFY_FRAME_CONCURRENCY = normalizeConcurrency(
  process.env.ANALYSIS_VERIFY_FRAME_CONCURRENCY,
  DEFAULT_INDEX_SEGMENT_CONCURRENCY,
);
const INDEX_SHARD_SECONDS = normalizePositiveNumber(process.env.ANALYSIS_INDEX_SHARD_SECONDS, 24, 4, 120);
const DESCRIPTION_BATCH_SIZE = 8;
const SUPABASE_INSERT_BATCH_SIZE = 250;

function clamp(value, min, max) {
  return Math.max(min, Math.min(max, value));
}

function normalizeConcurrency(value, fallback, max = 12) {
  if (!value) return clamp(fallback, 1, max);
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return clamp(fallback, 1, max);
  return clamp(Math.floor(parsed), 1, max);
}

function normalizePositiveNumber(value, fallback, min, max) {
  if (!value) return clamp(fallback, min, max);
  const parsed = Number(value);
...
function buildFrameShards(duration, intervalSeconds) {
  if (!duration || duration <= 0 || INDEX_SEGMENT_CONCURRENCY === 1) {
    return [{ startTime: 0, duration }];
  }

  const alignedShardSeconds = Math.max(
    intervalSeconds,
    roundTime(Math.round(INDEX_SHARD_SECONDS / intervalSeconds) * intervalSeconds),
  );

  if (duration <= alignedShardSeconds) {
    return [{ startTime: 0, duration }];
  }

  const shards = [];
  for (let startTime = 0; startTime < duration; startTime += alignedShardSeconds) {
    const shardDuration = Math.min(alignedShardSeconds, duration - startTime);
    if (shardDuration <= 0) continue;
    shards.push({
      startTime: roundTime(startTime),
      duration: roundTime(shardDuration),
    });
  }
  return shards;
}

async function extractFrameShard({
  filePath,
  outputDir,
  intervalSeconds,
  scaleWidth,
  ...
  return Promise.all(frameNames.map(async (name, index) => {
    ...
    return {
      index,
      sourceTime: roundTime(startTime + index * intervalSeconds),
      fileName,
...
export async function extractFramesAtInterval(filePath, outputDir, intervalSeconds, scaleWidth = 320, options = {}) {
  const duration = typeof options.duration === 'number' && Number.isFinite(options.duration)
    ? options.duration
    : null;
  const shards = buildFrameShards(duration, intervalSeconds);
  const shardResults = await mapWithConcurrency(
    shards,
    INDEX_SEGMENT_CONCURRENCY,
    async (shard, index) => extractFrameShard({
      filePath,
      outputDir: path.join(outputDir, `shard_${String(index + 1).padStart(3, '0')}`),
      intervalSeconds,
      scaleWidth,
  ...
  const merged = shardResults.flat().sort((a, b) => a.sourceTime - b.sourceTime);
  const deduped = [];
  for (const sample of merged) {
    const previous = deduped[deduped.length - 1];
    if (previous && Math.abs(previous.sourceTime - sample.sourceTime) < intervalSeconds / 2) {
      continue;
    }
    deduped.push({
      ...sample,
...
async function insertRowsInBatches(supabase, table, rows) {
  if (rows.length === 0) return;
  for (const batch of chunk(rows, SUPABASE_INSERT_BATCH_SIZE)) {
...
export async function indexAssetFromStorage(supabase, asset) {
  ...
  try {
    ...
    const samples = await extractFramesAtInterval(download.filePath, sampleDir, sampleInterval, 320, { duration });
    ...
    const descriptionBatches = chunk(representativeFrames, DESCRIPTION_BATCH_SIZE);
    const descriptions = (await mapWithConcurrency(
      descriptionBatches,
      INDEX_DESCRIPTION_CONCURRENCY,
      (batch) => describeFrameBatch(batch),
    )).flat();
    ...
    const [sceneDelete, visualDelete, transcriptDelete] = await Promise.all([
      supabase.from('asset_scenes').delete().eq('asset_id', asset.id),
      supabase.from('asset_visual_index').delete().eq('asset_id', asset.id),
      supabase.from('asset_transcript_words').delete().eq('asset_id', asset.id),
    ]);
    if (sceneDelete.error) throw sceneDelete.error;
    if (visualDelete.error) throw visualDelete.error;
    if (transcriptDelete.error) throw transcriptDelete.error;
    ...
    if (scenes.length > 0) {
      ...
      await insertRowsInBatches(supabase, 'asset_scenes', sceneRows);
    }
    ...
    await insertRowsInBatches(supabase, 'asset_visual_index', visualRows);
    ...
    return {
      ...
      sampleInterval,
      shardCount: buildFrameShards(duration, sampleInterval).length,
      segmentConcurrency: INDEX_SEGMENT_CONCURRENCY,
      descriptionConcurrency: INDEX_DESCRIPTION_CONCURRENCY,
    };
  } finally {
...
export async function verifyCandidatesAgainstQuery(supabase, asset, query, candidates) {
  ...
  const candidatesToVerify = candidates.slice(0, 6);
  const download = await downloadAssetToTempFile(supabase, asset, 'autocut-verify');
  try {
    const verificationFrames = (await mapWithConcurrency(
      candidatesToVerify,
      VERIFY_FRAME_CONCURRENCY,
      async (candidate) => {
        ...
        const offsets = [0.2, 0.5, 0.8].map((ratio) => roundTime(candidate.sourceStart + span * ratio));
        const frames = [];
    ...
    return candidatesToVerify.flatMap((candidate) => {
